...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', patience=10, mode='auto', verbose=1,
                              factor=0.8)
tb = TensorBoard(log_dir='c:/stduy_/_save/_tensorboard/_graph',
                 histogram_freq=0,
                 write_graph=True,
                 write_images=True,
                 )
...
import time
start = time.time()
hist = model.fit(x_train, y_train, epochs=30, batch_size=256,
...
path='./_save/keras/'
# model.save(hist, path + 'keras70_1_mnist_graph.h5')

print(hist)
# <keras.callbacks.History object at 0x0000020504195B50>
print(hist.history)
