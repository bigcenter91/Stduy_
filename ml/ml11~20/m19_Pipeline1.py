...
model = Pipeline([("std", StandardScaler()),("svc", SVC())])  #->얘는 리스트로 받아놈.

#TypeError: __init__() takes 2 positional arguments(객체) but 3 were given 
# 리스트로 해도 에러가뜸 그래서 튜플의 형태로 만들어줌, 문자열로도 해줘야됨.
