...
# pca는 데이터의 방향성에 선을 긋는다 평탄화 시킨다음 지표를 다시 잡고_비지도 학습_n_components(1을 넣으면 y의 컬럼 하나) // pca를 정의를하고
# pca = fit하고 진행 // x만 들어가지 
# lda는 각 데이터의 클래스별로 매칭을 시킨다_지도 학습: y의 클래스를 알기 때문에 또 갈리는게 명확함 (fit_transform하더라도 y를 넣어줘야한다)
