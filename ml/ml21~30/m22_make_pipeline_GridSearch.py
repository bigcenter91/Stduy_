...
# parameters = [{'randomforestclassifier__n_estimators' : [100, 200, 300]}, {'randomforestclassifier__max_depth' : [6, 10, 15, 12]}, 
#             {'randomforestclassifier__min_samples_leaf' : [3, 10]},
#     {'randomforestclassifier__min_samples_split' : [2, 3, 10]}, 
#     {'randomforestclassifier__max_depth' : [6, 8, 12]}, 
...
# #pipeline에 파라미터를 넣어야하는데 랜포 파라미터를 넣어서 오류가뜸. 해결책 randomForestClassifier__(정의한 것)을 넣으면됨.
...
#2. 모델
pipe = make_pipeline(StandardScaler(), RandomForestClassifier())
model = GridSearchCV(pipe, parameters, cv=5, verbose=1, n_jobs=3, scoring='accuracy')
