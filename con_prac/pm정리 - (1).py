    ...
    for k in range(64):
        for i in range(120):
            if np.isnan(test_pm[j, 120*k+i, 1]) and i<84:
                ...
                test_pm[j, 120*k+204-i-1, 1] = model2.predict(np.flip(np.concatenate([test_pm[j, 120*k+204-i:120*k+204-i+10+timesteps, :], test_pm_aws[j, 120*k+204-i:120*k+204-i+10+timesteps, :]], axis=1), axis=0).reshape(-1, timesteps, x.shape[2]).astype(np.float32))
