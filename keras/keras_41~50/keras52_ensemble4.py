...
y1 = np.array([range(2001, 2101)]).T # 환율
y2 = np.array([range(1001, 1101)]).T # 금리
...
print(x1_train.shape, y1_train.shape, y2_train.shape)
...
merge1 = Concatenate()([output1], name='mg1') # 2개 이상은 뭐? 리스트! 리스트 형태로 받아들일거다
#Convatenate 뒤에 () 붙히는 이유는 대문자는 클래스인데 클래스를 호출 할 때는 ()를 붙혀줘야한다 인스턴스?

# merge2 = Dense(2, activation='relu', name='mg2')(merge1)
# merge3 = Dense(3, activation='relu', name='mg3')(merge2)
hidden_output = Dense(1, name='last')(merge1)
