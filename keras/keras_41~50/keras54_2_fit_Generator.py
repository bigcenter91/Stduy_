...
xy_train = train_datagen.flow_from_directory(
    ...
    target_size=(100,100),
    batch_size=160, # 전체 데이터 쓸려면 160 넣어라 한방에 할려면 그냥 크게 잡아서 하면 된다 전체 데이터 갯수 이상을 늘려라
...
# model.fit(xy_train[:][0], xy_train[:][1], epoch=10) # [0][0]을하면 batch 때문에 다섯개만 들어가겠지
# TypeError: '>=' not supported between instances of 'slice' and 'int' 슬라이싱 : 넣지말고 데이터를 넣으라는 얘기

# model.fit(xy_train[0][0], xy_train[0][1], epochs=10) # [0][0]을하면 batch 때문에 다섯개만 들어가겠지
#얘도 통배치 넣으면 가능하지?
...
# plt.plot (hist.history['val_loss'], marker='.', c='red', label='로스')
# plt.plot (hist.history['loss'], marker='.', c='blue', label='발로스')
# plt.xlabel('epochs')
...
# plt.subplot(1,2,1)
# plt.plot (hist.history['val_acc'], marker='.', c='red', label='로스')
# plt.plot (hist.history['acc'], marker='.', c='blue', label='발로스')
# plt.xlabel('epochs')
